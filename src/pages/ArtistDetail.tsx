...
export default function ArtistDetail() {
  ...
  if (!artist) {
    return (
      <div className="flex flex-col min-h-svh bg-arena">
        <SEO title="Artista no encontrado" description="El artista que buscas no está en el cartel de Vettonia 2026." noindex />
        <HamburgerNav />
  ...
  const artistSchema = {
    ...
    startDate: `2026-08-14T${artist.time}:00`,
  ...
  return (
    <div className="flex flex-col min-h-svh bg-arena">
      <SEO
        title={artist.name}
        description={`${artist.name} en Vettonia 2026. ${artist.stage} — ${artist.time}. ${artist.genre}.`}
      ...
      <div className="flex-1">
        <section className="px-5 py-16 sm:py-20 relative overflow-hidden min-h-[80vh] flex items-center">
          ...
          <span className="absolute font-heading text-[clamp(8rem,30vw,25rem)] font-extrabold text-violeta/3 leading-none tracking-[-0.08em] select-none pointer-events-none top-[-5%] right-[-5%]">
            {artist.name.split(' ')[0]}
          </span>

          <SolidBox className="w-14 h-14 bg-coral/40 left-[4%] top-[6%] z-30 rotate-20" />
          <SolidRing className="w-28 h-28 border-violeta/20 right-[-5%] top-[6%] z-30" />
          <SolidDot className="w-7 h-7 bg-violeta/40 right-[12%] top-[40%] z-30" />
          <SolidLine className="w-36 h-0.5 bg-coral/30 left-[15%] bottom-[35%] z-30" />
          <SolidBox className="w-9 h-9 bg-coral/35 left-[50%] bottom-[10%] z-30 rotate-40" />
          <SolidDot className="w-5 h-5 bg-white/30 right-[30%] bottom-[20%] z-30" />
